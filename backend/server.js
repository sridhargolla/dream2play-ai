...
const ERROR_TRANSLATIONS = {
  hi: {
    ...
    "Please provide dream title and description":
      "कृपया सपने का शीर्षक और विवरण प्रदान करें",
    "Failed to generate game blueprint": "गेम ब्लूप्रिंट उत्पन्न करने में विफल",
    "Please select two dreams to fuse":
      "कृपया फ्यूज करने के लिए दो सपनों का चयन करें",
    "One or both dreams not found": "एक या दोनों सपने नहीं मिले",
    "Unauthorized to fuse these dreams":
      "इन सपनों को फ्यूज करने के लिए अनधिकृत",
  ...
  te: {
    ...
    "User already exists with this email":
      "ఈ ఈమెయిల్‌తో వినియోగదారు ఇప్పటికే ఉన్నారు",
    ...
    "Please provide dream title and description":
      "దయచేసి కల శీర్షిక మరియు వివరణను అందించండి",
    "Failed to generate game blueprint":
      "గేమ్ బ్లూప్రింట్ సృష్టించడంలో విఫలమైంది",
    "Please select two dreams to fuse":
      "దయచేసి ఫ్యూజ్ చేయడానికి రెండు కలలను ఎంచుకోండి",
    "One or both dreams not found": "ఒకటి లేదా రెండు కలలు కనుగొనబడలేదు",
    "Unauthorized to fuse these dreams":
      "ఈ కలలను ఫ్యూజ్ చేయడానికి మీకు అధికారం లేదు",
    "Failed to fuse dreams": "కలలను ఫ్యూజ్ చేయడంలో విఫలమైంది",
    "Unauthorized to delete this dream":
      "ఈ కలను తొలగించడానికి మీకు అధికారం లేదు",
    ...
    "Please provide all score details":
      "దయచేసి అన్ని స్కోర్ వివరాలను అందించండి",
...
const getAIConfig = (req) => {
  return {
    provider:
      req.headers["x-ai-provider"] || req.body?.aiConfig?.provider || "openai",
    apiKey:
      req.headers["x-ai-key"] ||
      req.headers["x-openai-key"] ||
      req.body?.aiConfig?.apiKey ||
      "",
    model: req.headers["x-ai-model"] || req.body?.aiConfig?.model || "",
    endpoint:
      req.headers["x-ai-endpoint"] || req.body?.aiConfig?.endpoint || "",
  };
};
...
app.post("/api/dreams/fuse", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const fusedDescription = `A hybrid dream fusing "${dream1.title}" and "${
      dream2.title
    }". Description: ${dream1.description.slice(0, 50)}... and ${dream2.description.slice(
      0,
      50,
    )}...`;
...
app.post("/api/scores", authenticateToken, (req, res) => {
  ...
  try {
    ...
    const newScore = saveScore({
      ...
      completionTime: Number.parseFloat(completionTime),
      difficulty,
    });
