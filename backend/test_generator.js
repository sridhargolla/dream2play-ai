...
const testPrompts = [
  ...
  // Shooter/Survival
  {
    ...
    description: 'Blast alien spaceships using laser cannons and weapon upgrades inside Hangar 9.',
  ...
  // Driving
  {
    ...
    description: 'A man driving a red Ferrari through Mumbai traffic escaping police.',
  ...
  // Bike Racing
  {
    ...
    description: 'A bike race through Tokyo at night dodging corporate speed vehicles.',
  ...
  // Battle Royale
  {
    ...
    description: 'Free Fire style battle royale in Dubai, looting weapons and surviving the shrinking wall.',
  ...
  // Endless Runner
  {
    ...
    description: 'Endless runner in Hyderabad, switching lanes and jumping hurdles while collecting gold coins.',
...
async function runTests() {
  ...
  for (let i = 0; i < testPrompts.length; i++) {
    ...
    console.log(`[TEST ${i + 1}/${testPrompts.length}] Generating: "${prompt.title}"...`);

    const blueprint = await analyzeDream(prompt.title, prompt.description, null);

    console.log(` -> Title: "${blueprint.title}"`);
    console.log(` -> Genre Classified: "${blueprint.genre}" (Expected: "${prompt.expectedGenre}")`);
    console.log(` -> Location: "${blueprint.intent.location}" (Expected: "${prompt.expectedLocation}")`);
    ...
    console.log(` -> Stage 1 Environment: "${blueprint.stages[0].environment}"`);
    console.log(` -> Stage 1 Blocks Count: ${blueprint.stages[0].blocks.length}`);
    console.log(` -> Stage 1 Enemies Count: ${blueprint.stages[0].enemies.length}`);
    if (blueprint.stages[0].enemies.length > 0) {
      console.log(`    - First Enemy Type: "${blueprint.stages[0].enemies[0].type}"`);
    }
    console.log('------------------------------------------');
    ...
    if (
      ...
      assert.strictEqual(blueprint.player.gravity, 0, 'Gravity must be 0 for runner/racing genres');
    } else {
      assert(blueprint.player.gravity > 0, 'Gravity must be positive for standard platformer/survival genres');
    }

    // Verify stage blocks have correct structures based on genre
    blueprint.stages.forEach((stage) => {
      if (blueprint.genre.includes('driving') || blueprint.genre.includes('racing')) {
        const roadBlocks = stage.blocks.filter((b) => b.type === 'ground');
        assert(roadBlocks.length > 0, 'Driving game must have ground road blocks');
        const trafficEnemies = stage.enemies.filter((e) => e.type === 'traffic');
        assert(trafficEnemies.length > 0, 'Driving game must have traffic obstacles');
      }
  ...
  assert(bMumbai.player.colors.bg !== bTokyo.player.colors.bg, 'Mumbai and Tokyo should have different backgrounds');
  assert(bTokyo.player.colors.bg !== bDubai.player.colors.bg, 'Tokyo and Dubai should have different backgrounds');
