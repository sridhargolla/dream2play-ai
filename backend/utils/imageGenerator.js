...
function buildFallbackSvg({ assetType, blueprint, dreamContext = {} }) {
  ...
  const enemyName = (blueprint.enemies && blueprint.enemies[0]) || 'Enemy';
  const bossName = blueprint.boss || 'Boss';
  const worldName = blueprint.world || 'Dream World';
  const collectibleName = (blueprint.powerups && blueprint.powerups[0]) || 'Collectible';
  ...
  const dreamText = [dreamContext.title, dreamContext.description].filter(Boolean).join(' ');
  const detail = dreamText
    ? `<text x="50%" y="88%" text-anchor="middle" font-size="24" fill="white" opacity="0.85">${dreamText.slice(0, 70)}</text>`
    : '';
...
async function writeFallbackSvg(destPath, { assetType, blueprint, dreamContext }) {
  const svg = buildFallbackSvg({ assetType, blueprint, dreamContext });
  await fs.promises.writeFile(destPath, svg, 'utf8');
}

async function generateAsset({ prompt, apiKey, width, height, destPath, assetType, blueprint, dreamContext }) {
  let imageUrl = null;
  ...
  if (apiKey) {
    try {
      ...
      console.warn(`Error in OpenAI generation: ${err.message}. Falling back to Pollinations AI.`);
    }
  }
  ...
  try {
    ...
    console.warn(`Remote image generation failed: ${err.message}. Writing a local high-quality SVG fallback.`);
    const fallbackPath = destPath.replace(/\.png$/i, '.svg');
    await writeFallbackSvg(fallbackPath, { assetType, blueprint, dreamContext });
    return fallbackPath;
  }
}
...
function buildAssetPrompt(assetType, blueprint, dreamContext = {}) {
  const heroName = blueprint.hero || 'Hero';
  const enemyName = (blueprint.enemies && blueprint.enemies[0]) || 'Monster';
  const bossName = blueprint.boss || 'Boss Overlord';
  const worldName = blueprint.world || 'Surreal Void';
  const collectibleName = (blueprint.powerups && blueprint.powerups[0]) || 'energy crystal';
 ...
 */
async function generateAndSaveAssets(blueprint, dreamId, apiKey, dreamContext = {}) {
  const assetSpecs = {
    hero: {
