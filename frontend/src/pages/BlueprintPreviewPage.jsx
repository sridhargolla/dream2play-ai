    ...
    <div className="w-full max-w-6xl mx-auto py-8 px-4 flex flex-col gap-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="text-left">
          ...
          <p className="text-sm text-gray-400 mt-2 max-w-2xl">{dream.description}</p>
        </div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="glass-panel p-6 rounded-2xl border border-white/10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                <span className="text-gray-500 uppercase font-bold text-[10px]">{t('dreamTitle')}</span>
              ...
              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                <span className="text-gray-500 uppercase font-bold text-[10px]">{t('hero')}</span>
                <p className="text-white font-bold mt-1">{bp.hero || bp.player?.name || 'Explorer'}</p>
              </div>
              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                <span className="text-gray-500 uppercase font-bold text-[10px]">{t('difficulty')}</span>
                <p className="text-yellow-400 font-bold mt-1 uppercase">{bp.difficulty || 'Medium'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10">
            ...
            <div className="flex flex-col gap-4">
              {(stages.length ? stages : [{ environment: bp.world, objective: bp.objective, boss: { name: bp.boss } }]).map(
                (stage, idx) => (
                  <div
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-[10px] text-gray-500 font-mono">{stage.environment || bp.world}</span>
                    </div>
                    ...
                    {stage.boss && (
                      <p className="text-xs text-red-300 flex items-center gap-2">
                        ...
                        {t('boss')}: <span className="font-bold text-red-400">{stage.boss.name || bp.boss}</span>
                        {stage.boss.maxHp ? (
                          <span className="text-gray-500">({stage.boss.maxHp} HP)</span>
                        ) : null}
                      </p>
                    )}
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10">
            ...
            <div className="flex flex-col gap-3 text-xs">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="text-gray-500 uppercase font-bold text-[10px]">{t('world')}</span>
                  <p className="text-white font-semibold">{bp.world || bp.theme}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <Swords className="w-4 h-4 text-red-400 shrink-0" />
                <div>
                  <span className="text-gray-500 uppercase font-bold text-[10px]">{t('enemies')}</span>
                  <p className="text-white font-semibold">{(bp.enemies || []).join(', ') || 'Dream shadows'}</p>
                </div>
              </div>
              {bp.genre === 'puzzle' && (
                <div className="flex items-start gap-2">
            ...
            {bp.stories && (
              <div className="mt-4 pt-4 border-t border-white/5 text-xs text-gray-300 leading-relaxed flex flex-col gap-2">
                ...
                <p className="text-white font-semibold border-l-2 border-[var(--accent-color)] pl-2">{bp.stories.mission}</p>
              </div>
            )}
          </div>
